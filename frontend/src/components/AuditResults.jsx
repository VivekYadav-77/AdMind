...
import { AlertTriangle, MapPin, Monitor, Users, TrendingDown, ArrowRight, BarChart3, Bot, Target, AlertCircle } from 'lucide-react'
...
export default function AuditResults({ audit }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('threshold_wasteAlert')
    if (saved) setWasteThreshold(parseInt(saved))
  }, [])
  ...
  const stats = [
    ['Total Spend', money(audit.total_spend), 'text-textprimary'],
  ...
  return (
    ...
    >
      <div className="glass-panel rounded-[2.5rem] p-6 lg:p-10 border-borderwarm relative overflow-hidden shadow-2xl backdrop-blur-2xl bg-bgpanel">
        ...
        <motion.div variants={itemVariants} className="flex items-center gap-5 mb-10 relative z-10">
          ...
          <div>
            <h2 className="text-3xl font-black text-textprimary tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-textprimary to-textmuted">Audit Intelligence</h2>
            <p className="text-textmuted text-sm mt-1 font-medium">Deep-dive analysis of your campaign performance</p>
          </div>
        </motion.div>

        {/* AI Structured Summary */}
        <motion.div variants={containerVariants} className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-12 relative z-10">
          {/* Overview Card */}
          <motion.div 
            variants={itemVariants}
            className="relative rounded-3xl bg-gradient-to-r from-blue-900/10 to-indigo-900/10 border border-blue-400/20 p-6 text-[14px] leading-relaxed text-textprimary shadow-[0_0_20px_rgba(59,130,246,0.1)] backdrop-blur-xl overflow-hidden group"
          >
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="font-medium text-textsecondary">{audit.summary?.overview}</p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div 
            variants={itemVariants}
            className="relative rounded-3xl bg-gradient-to-r from-red-900/10 to-orange-900/10 border border-red-500/20 p-6 text-[14px] leading-relaxed text-textprimary shadow-[0_0_20px_rgba(239,68,68,0.1)] backdrop-blur-xl overflow-hidden group"
          >
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="font-medium text-textsecondary">{audit.summary?.critical_finding}</p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div 
            variants={itemVariants}
            className="relative rounded-3xl bg-gradient-to-r from-emerald-900/10 to-teal-900/10 border border-emerald-500/20 p-6 text-[14px] leading-relaxed text-textprimary shadow-[0_0_20px_rgba(16,185,129,0.1)] backdrop-blur-xl overflow-hidden group"
          >
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="font-medium text-textsecondary">{audit.summary?.action_required}</p>
              </div>
            </div>
          </motion.div>
        </motion.div>
        ...
        {/* Top Stats */}
        <motion.div variants={containerVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-10 relative z-10">
          {stats.map(([label, value, colorClass], i) => (
            <motion.div 
              variants={itemVariants}
              key={label} 
              className="relative rounded-3xl border border-borderwarm bg-bgpanelhover p-6 shadow-lg hover:shadow-[0_8px_30px_rgba(0,0,0,0.1)] transition-all overflow-hidden group backdrop-blur-lg"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              <p className="text-xs font-bold uppercase tracking-widest text-textmuted mb-2">{label}</p>
        ...
        {/* Charts Row */}
        <motion.div variants={containerVariants} className="grid lg:grid-cols-3 gap-6 mb-12 relative z-10">
          {/* Spend Efficiency Pie Chart */}
          <motion.div variants={itemVariants} className="rounded-3xl border border-borderwarm bg-bgpanelhover p-8 shadow-lg flex flex-col items-center backdrop-blur-lg">
            <h3 className="text-sm font-black text-textsecondary w-full text-left mb-6 uppercase tracking-widest">Spend Efficiency</h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip 
                    ...
                    contentStyle={{ backgroundColor: 'var(--bg-panel)', backdropFilter: 'blur(10px)', borderColor: 'var(--border-warm)', borderRadius: '16px', color: 'var(--text-primary)', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.5)' }}
                    itemStyle={{ color: 'var(--text-primary)', fontWeight: 'bold' }}
                  />
                </PieChart>
              </ResponsiveContainer>
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-3xl font-black text-textprimary drop-shadow-md">{audit.total_spend > 0 ? Math.round((efficientSpend / audit.total_spend) * 100) : 0}%</span>
                <span className="text-[10px] uppercase tracking-widest text-textmuted font-bold">Efficient</span>
              </div>
            </div>
          ...
          <motion.div variants={itemVariants} className="lg:col-span-2 rounded-3xl border border-borderwarm bg-bgpanelhover p-8 shadow-lg flex flex-col backdrop-blur-lg">
            <h3 className="text-sm font-black text-textsecondary mb-6 uppercase tracking-widest">Highest Impact Issues (by Spend)</h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topWasted} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barSize={36}>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ backgroundColor: 'var(--bg-panel)', backdropFilter: 'blur(10px)', borderColor: 'var(--border-warm)', borderRadius: '16px', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.5)' }}
                    labelStyle={{ color: 'var(--text-secondary)', marginBottom: '6px', fontWeight: 'bold' }}
        ...
        <motion.div variants={containerVariants} className="mb-12 relative z-10">
          <motion.h3 variants={itemVariants} className="text-xl font-black text-textprimary mb-6 flex items-center gap-3">
          ...
          <div className="grid gap-4">
            {audit.issues.map((issue, index) => {
              const isOverThreshold = audit.total_spend > 0 && ((issue.spend / audit.total_spend) * 100) > wasteThreshold
              ...
              return (
                <motion.div 
                  ...
                  className={clsx(
                    "group flex flex-col md:flex-row md:items-center gap-5 rounded-2xl border p-5 transition-all backdrop-blur-md shadow-sm hover:shadow-lg",
                    isOverThreshold ? "bg-red-500/5 border-red-500/30" : "bg-bgpanelhover border-borderwarm hover:bg-bgpanel"
                  )}
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-3 mb-2">
                      <h4 className="text-lg font-black text-textprimary truncate">{issue.keyword}</h4>
                      <span className="text-xs font-bold text-textsecondary bg-bgpanel px-3 py-1 rounded-lg border border-borderwarm shadow-inner">
                        {issue.campaign_name}
                      </span>
                      ...
                      {isOverThreshold && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-red-400 bg-red-500/10 px-2 py-1 rounded-lg">
                          <AlertCircle size={12} /> High Waste
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-textsecondary font-medium">{issue.detail}</p>
                  </div>
                  
                  <div className="flex items-center gap-6 shrink-0 md:ml-4 bg-bgpanel p-4 rounded-xl border border-borderwarm shadow-inner">
                    <div>
                      <p className="text-[10px] uppercase tracking-widest text-textmuted font-black mb-1">Issue Type</p>
                      <p className="text-sm font-bold text-textsecondary capitalize">{issue.issue_type.replace(/_/g, ' ')}</p>
                    </div>
                    <div className="text-right border-l border-borderwarm pl-6">
                      <p className="text-[10px] uppercase tracking-widest text-textmuted font-black mb-1">Impacted Spend</p>
            ...
            <motion.div variants={itemVariants} className="mb-6 flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <h3 className="text-xl font-black text-textprimary tracking-tight">Hidden Segment Anomalies</h3>
              ...
              {anomalies.map((anomaly, index) => {
                ...
                return (
                  <motion.div
                    ...
                    key={`anomaly-${index}`}
                    className="relative flex flex-col rounded-3xl border border-amber-500/30 bg-gradient-to-br from-amber-500/5 to-bgpanel p-6 shadow-xl overflow-hidden group backdrop-blur-xl"
                  >
                    ...
                    <div className="flex items-center justify-between mb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="inline-flex items-center gap-1.5 rounded-lg bg-bgpanel px-3 py-1.5 text-xs font-black uppercase tracking-widest text-textsecondary border border-borderwarm shadow-inner">
                    ...
                    <div className="mb-3 flex flex-wrap items-baseline gap-2 bg-bgpanel p-3 rounded-xl border border-borderwarm shadow-inner">
                      <ArrowRight size={16} className="text-textmuted" />
                      <span className="text-base font-black text-textprimary">{anomaly.keyword}</span>
                      <span className="text-xs font-bold text-textmuted">in {anomaly.campaign_name}</span>
                    </div>
                    
                    <p className="text-sm text-textsecondary mt-2 flex-1 leading-relaxed font-medium">{anomaly.detail}</p>
                    
                    <div className="mt-6 pt-5 border-t border-amber-500/20 flex justify-between items-center text-xs font-black">
                      <span className="text-textmuted uppercase tracking-widest">Impacted Segment Spend</span>
